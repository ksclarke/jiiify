...
The [README](https://github.com/ksclarke/jiiify/blob/master/README.md) in the project's GitHub repository will help you get Jiiify started, but you may want to configure the administrative interface's authentication system to use your own Google OAuth client ID instead of the default FreeLibrary placeholder that can be used for testing on localhost. To do this, you will need to have a Google account and to add your Google email address and Google client ID to the [configuration file](https://github.com/ksclarke/jiiify/blob/master/src/main/resources/sample-config.json) that's passed to Jiiify when it starts (Note: You will need to add your email address to this file even if you do not decide to configure a new Google OAuth client ID because the email addresses in there serve as a whitelist). For what it's worth, the current login system is just a temporary placeholder for a more developed authentication system. The stable release of Jiiify will probably offer multiple OAuth providers in addition to an htdigest option.
...
If you'd like, you can use the test CSV file found at `${project.basedir}/target/test-classes/csv/test.csv` after you've built the project. Its TIFF file should have been downloaded to your machine as a part of the build.
